...
public:
    int solve(int &n, int &l, int &r, vector<int> &a) {
        // dp[i] represents the maximum number of rounds that can be won
        // using cards from index i to the end
        vector<int> dp(n + 1, 0);
        
        // Start from the end of the array
        for (int i = n - 1; i >= 0; i--) {
            // Initialize with the best result possible without using current position
            dp[i] = dp[i + 1];
