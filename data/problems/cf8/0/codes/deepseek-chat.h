...
public:
    string solve(int &n, vector<int> &a, string &s) {
        // Create a map to store the mapping from numbers to characters
        unordered_map<int, char> numToChar;
        // Create a map to store the mapping from characters to numbers
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (numToChar.find(num) != numToChar.end()) {
                ...
                if (charToNum.find(ch) != charToNum.end()) {
                    ...
                    // If both are not mapped, create the mapping
                    numToChar[num] = ch;
                    charToNum[ch] = num;
                }
            }
