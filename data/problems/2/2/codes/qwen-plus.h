        ...
        for (int i = 1; i <= N; ++i) {
            for (int j = 0; j <= V; ++j) {
                ...
                if (v[i - 1] > j) {
                    ...
                    // Otherwise, consider both options: taking the item or not taking it
...
1. **Initialization**:
   - We initialize a 2D vector `dp` of size `(N+1) x (V+1)` with all elements set to 0. This table will store the maximum value that can be achieved with a given number of items and a specific capacity.
...
This approach ensures that we explore all possible combinations of items while keeping track of the maximum value that can be achieved within the given constraints.
